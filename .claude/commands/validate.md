...
*Amended again 2026-08-13: **both halves of a `<time>` must name the same day.** The date rules above compare `datePublished` against the `datetime` ATTRIBUTE, and `_POST_TIME` read only that attribute — so when `longDate` formatted in the build machine's local zone while `isoDate` formatted in UTC, one element carried `datetime="2026-08-13">12 August 2026` and this check confirmed it agreed with itself. The 2026-08-12 dateline lesson in a new place: a check whose regex cannot see the markup it is checking reports zero and means nothing. It now captures the element's text as well and asserts it names the attribute's day, in either the long form or the ISO form — the DAY, not the format, since `/methodology/` legitimately prints the ISO string where the post pages print words. Checked on any page carrying a `<time>` rather than only on posts, because `/blog/` prints the same pair through the same two helpers. Verified against the real pre-fix output rather than only a fixture.*
